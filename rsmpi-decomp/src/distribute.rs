...
pub struct Distribution<const N: usize> {
    // Size of data of current processors
    pub sz: [usize; N],
    // Starting index of data of current processors
    pub st: [usize; N],
    // Ending index of data of current processors
    pub en: [usize; N],
...
mod tests {
    ...
    fn check_slab_size() {
        let (nx, ny) = (4, 2);
        let xdist = Distribution::new([nx, ny], 2, 1);
        assert_eq!(xdist.sz, vec![[4, 1]; 2]);
        let ydist = Distribution::new([nx, ny], 2, 0);
        assert_eq!(ydist.sz, vec![[2, 2]; 2]);
    }
}
